...
const Logistics = () => {
  ...
  useEffect(() => {
    const loadAssets = async () => {
      setLoading(true);
      const pubLogAssets = await getAssets();
          ...
          <div className="lg:col-span-1 space-y-4 h-[60vh] overflow-y-auto pr-4">
            {loading ? (
              ...
              filteredAssets.map(asset => (
                <div 
                  key={asset.id} 
                  className={`bg-black/20 p-4 rounded-lg cursor-pointer border-2 ${selectedAsset?.id === asset.id ? 'border-blue-500' : 'border-transparent'}`}
          ...
          <div className="lg:col-span-2 bg-white/10 backdrop-blur-md rounded-lg shadow-lg p-6 h-[60vh] overflow-y-auto">
            {selectedAsset ? (
              <div>
                ...
                <div className="mt-6 h-80 rounded-lg overflow-hidden">
                  <MapComponent 
